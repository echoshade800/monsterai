...
# 更新 Agent 配置脚本
# 用法: ./update_miniapp_config.sh [dev|prod]
# 示例: ./update_miniapp_config.sh dev   (更新开发环境)
#       ./update_miniapp_config.sh prod  (更新生产环境，谨慎操作!)
...
# 根据环境设置变量
if [ "$ENV" == "dev" ]; then
    CONFIG_FILE="miniapp_list_config_debug.json"
    echo "🔄 更新开发环境配置..."
else
    CONFIG_FILE="miniapp_list_config_prod.json"
...
# 配置文件路径
LOCAL_FILE="./assets/config/miniapps/${CONFIG_FILE}"
